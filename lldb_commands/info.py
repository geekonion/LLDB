...
def handle_command(debugger, command, exe_ctx, result, internal_dict):
    ...
    if options.verbose:
        ...
        if target.GetProcess().GetMemoryRegionInfo(address, memRegion).success:
            ...
            memString += " {}{}{}".\
                format("R" if memRegion.IsReadable() else "-",
                       "W" if memRegion.IsWritable() else "-",
                       "X" if memRegion.IsExecutable() else "-")

    if foundAddress:
        result.AppendMessage('{}{}, {} {}'.
                             format("&" if options.address_of else "", args[0], returnDescription, memString))
    else:
...
def tryMachOAddress(addr, target, options):
    ...
    if module.IsValid():
        sectionName = " `{}`{}".\
            format(ds.attrStr(addr.GetModule().GetFileSpec().GetFilename(), 'cyan'),
...
def tryHeapAddress(addr, target, options):
    ...
    if ((void*)malloc_zone_from_ptr(ptr)) {
        retString = [NSString stringWithFormat:@"%p heap pointer, (0x%x bytes), zone: %p", ptr, (size_t)malloc_good_size((size_t)malloc_size(ptr)), (void*)malloc_zone_from_ptr(ptr)];
    }
    ...
    if not val.GetError().success \
            or val.GetValueAsUnsigned() == 0 \
            or not val.description \
            or val.description.find('error: ') != -1:
        return False, ""
...
def generate_option_parser():
    ...
    parser.add_option("-a", "--address_of",
                      ...
                      default=False,
                      dest="address_of",
                      help="By default Swift makes it a pain the ass to get the address a variable, "
